...
public class HollywoodWithZ 
{
    private static WordFrequencyCounter wfCounter;
	private static WordWithZ wordWithZ;
    ...
    public static class WordFrequencyCounter implements Framework 
    {
        ...
        private ArrayList<Pairs> pairs = new ArrayList<Pairs>();
        ...
        public void run() throws IOException 
        {
            ...
            {
                System.out.println(pairs.get(i).s + " - " + pairs.get(i).number);//output final results
            }
        }
    }
    
    public static class WordWithZ implements Framework 
    {
        private HashMap<String, Integer> wordWithZMap = new HashMap<String, Integer>();
        
        public WordWithZ(WordFrequencyFrameWork wfapp, DataStorage dataStorage) 
        {
            dataStorage.registerForWordEvent(this);
            wfapp.registerForEndEvent(this);
        }
        ...
        public void load(String word) throws IOException 
        {
            if (word.contains("z")) 
            {
                if (wordWithZMap.containsKey(word)) 
                {
                    wordWithZMap.put(word, wordWithZMap.get(word) + 1);
                }
                else 
                {
                    wordWithZMap.put(word, 1);
                }
            }
        }
        public int getSize()
        {
        	int total = 0;
        	java.util.Iterator<Entry<String, Integer>> it = wordWithZMap.entrySet().iterator();
            while (it.hasNext()) {
        ...
        public void run() throws IOException 
        {
            System.out.println("Words with Z : " + getSize());
        }
    }
    ...
    public static void main(String[] args) throws IOException 
    {
        ...
        wordWithZ = new WordWithZ(wfapp, dtStorage);
        wfapp.run(args[0]);
    }
}